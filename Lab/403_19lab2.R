## -------------------------------------------------------------
## Stat/Q Sci 403 Lab 2 | Spring 2019 | University of Washington
## -------------------------------------------------------------
###              ###
### Part 1: CDF  ###
###              ###
...
x_base2 <- seq(-5, 5, 0.01)
plot(x_base, punif(x_base, min=-4, max=4), type = 'l', lwd=3, col="skyblue")
lines(x_base2, pexp(x_base2, 2), lwd=3, col='orange')
lines(x_base2, pnorm(x_base2, 0, 1), lwd=3, col='tan')

### Exercise 1.2: 
###  In one plot, show the CDF of Exp(1), Exp(2), Exp(5), Exp(10)
###  for x within [0, 4].
x_base3 <- seq(0, 4, 0.01)
plot(x_base3, pexp(x_base3, 1), type='l', lwd=3, col='skyblue')
lines(x_base3, pexp(x_base3, 2), lwd=3, col='blue')
lines(x_base3, pexp(x_base3, 5), lwd=3, col='orange')
lines(x_base3, pexp(x_base3, 10), lwd=3, col='tan')
...
x_base <- c(1,1,3,4,7)
x_edf <- ecdf(x_base)
plot(x_edf, do.points=F, col="skyblue", lwd=3)
...
x_edf2 <- ecdf(x_base2)
plot(x_edf, do.points=F, col='skyblue', lwd=3)
lines(x_edf2, do.points=F, col='orange')
...
# H0: data1 and data2 are from same distribution
ks.test(data1,data2)
...
data1_edf <- ecdf(data1)
data2_edf <- ecdf(data2)
plot(data1_edf, do.points=F, col='tan', lwd=3)
lines(data2_edf, do.points=F, col='blue', lwd=3)

p_exp_dist <- rep(NA, 100)
for (i in 1:100) {
  data1 <- rexp(100, 1)
  data2 <- rexp(100, 2)
  p_exp_dist[i] = ks.test(data1, data2)$p.value
}
hist(p_exp_dist, probability=T, col="skyblue")
...
data1 <- rexp(100, 1)
data2 <- qexp(runif(100), 1)
data1_edf <- ecdf(data1)
data2_edf <- ecdf(data2)

plot(data1_edf, do.point=F, lwd=3, col='red')
lines(data2_edf, do.point=F, lwd=3, col='blue')
